...
public class  Shader {
    ...
    private String read(String path) {
        try {
            var resource = MinecraftClient.getInstance().getResourceManager()
                .getResource(Identifier.of("pupper", "shaders/" + path))
                .orElseThrow(() -> new RuntimeException("No Found Shader File: assets/pupper/shaders/" + path));

            try (var in = resource.getInputStream()) {
                return IOUtils.toString(in, StandardCharsets.UTF_8);
            }
        } catch (IOException e) {
            throw new IllegalStateException("Read Shader Failed: " + path, e);
        }
    }
	...
	public void set(String name, Matrix4f mat) {
		mat.get(MAT);
		GlStateManager._glUniformMatrix4(getLocation(name), MAT);
	}
